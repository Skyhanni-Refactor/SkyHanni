...
object PlatformUtils {
    ...
    fun getModFromPackage(packageName: String?): ModInstance? =
        modPackages[packageName]?.let { ModInstance(it.modId, it.name, it.version) }

    fun exitJava(code: Int, hardExit: Boolean = false) {
        FMLCommonHandler.instance().exitJava(code, hardExit)
    }

    fun delayedExit(code: Int, waitForServer: Boolean = false) {
        FMLCommonHandler.instance().handleExit(code)
        if (waitForServer) {
